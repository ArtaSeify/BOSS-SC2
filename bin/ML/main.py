...
class Driver:
    def __init__(self, args):
        ...
        with open(BIN_PATH + "/" + self.experiment_file_name + ".txt", 'r') as experiment_file:
            ...
            self.experiment_names = list(self.experiment_data["Experiments"].keys())
            self.data_file_name = "LatestData"#self.experiment_name
            ...
            with open(BIN_PATH + "/" + self.experiment_file_name + "_modified.txt", 'w') as modified_exp_file:
                for experiment_name in self.experiment_names:
                    if self.load_name is None:
                        self.experiment_data["Experiments"][experiment_name]["UsePolicyNetwork"] = False
                        self.experiment_data["Experiments"][experiment_name]["UsePolicyValueNetwork"] = False
                    else:
                        if self.output_type == "B":
                            self.experiment_data["Experiments"][experiment_name]["UsePolicyValueNetwork"] = True
                            self.experiment_data["Experiments"][experiment_name]["UsePolicyNetwork"] = False
                        elif self.output_type == "P":
                            self.experiment_data["Experiments"][experiment_name]["UsePolicyValueNetwork"] = False
                            self.experiment_data["Experiments"][experiment_name]["UsePolicyNetwork"] = True

                    if len(self.experiment_names) == 1:
                        self.experiment_data["Experiments"][experiment_name]["OutputDir"] = BIN_PATH + "/" + experiment_name + "/" + str(self.start_run)
                    else:
                        self.experiment_data["Experiments"][experiment_name]["OutputDir"] = BIN_PATH + "/CombinedRun/" + experiment_name + "/" + str(self.start_run)
    ...
    def create_teststrength_file(self, run):
        ...
        with open(BIN_PATH + "/" + self.strengthtest_file_name + ".txt", 'w') as strength_exp_file:
            for experiment_name in self.experiment_names:
                ...
                if len(self.experiment_names) == 1:
                    strength_exp_json["Experiments"][experiment_name]["OutputDir"] = BIN_PATH + "/" + experiment_name + "/StrengthTest/WithReset/" + str(run)
                else:
                    strength_exp_json["Experiments"][experiment_name]["OutputDir"] = BIN_PATH + "/CombinedRun/" + experiment_name + "/StrengthTest/WithReset/" + str(run)
                
                strength_exp_json["Experiments"][experiment_name]["Run"][1] = self.numTestRuns
                if self.output_type == "B":
                    strength_exp_json["Experiments"][experiment_name]["UsePolicyValueNetwork"] = usePolicy
                    strength_exp_json["Experiments"][experiment_name]["UsePolicyNetwork"] = False
                elif self.output_type == "P":
                    strength_exp_json["Experiments"][experiment_name]["UsePolicyValueNetwork"] = False
                    strength_exp_json["Experiments"][experiment_name]["UsePolicyNetwork"] = usePolicy
                strength_exp_json["Experiments"][experiment_name]["SaveStates"] = False
                strength_exp_json["Experiments"][experiment_name]["ChangingRoot"]["Active"] = True
                strength_exp_json["Experiments"][experiment_name]["SearchParameters"]["TemperatureChangeFrame"] = 0

                # No reset after changing root
                strength_exp_json["Experiments"][experiment_name + "_2"] = copy.deepcopy(strength_exp_json["Experiments"][experiment_name])
                if len(self.experiment_names) == 1:
                    strength_exp_json["Experiments"][experiment_name + "_2"]["OutputDir"] = BIN_PATH + "/" + experiment_name + "/StrengthTest/WithoutReset/" + str(run)
                else:
                    strength_exp_json["Experiments"][experiment_name + "_2"]["OutputDir"] = BIN_PATH + "/CombinedRun/" + experiment_name + "/StrengthTest/WithoutReset/" + str(run)
                
                strength_exp_json["Experiments"][experiment_name + "_2"]["Run"][1] = self.numTestRuns
                if self.output_type == "B":
                    strength_exp_json["Experiments"][experiment_name + "_2"]["UsePolicyValueNetwork"] = usePolicy
                    strength_exp_json["Experiments"][experiment_name + "_2"]["UsePolicyNetwork"] = False
                elif self.output_type == "P":
                    strength_exp_json["Experiments"][experiment_name + "_2"]["UsePolicyValueNetwork"] = False
                    strength_exp_json["Experiments"][experiment_name + "_2"]["UsePolicyNetwork"] = usePolicy
                strength_exp_json["Experiments"][experiment_name + "_2"]["SaveStates"] = False
                strength_exp_json["Experiments"][experiment_name + "_2"]["ChangingRoot"]["Active"] = True
                strength_exp_json["Experiments"][experiment_name + "_2"]["ChangingRoot"]["Reset"] = False
                strength_exp_json["Experiments"][experiment_name + "_2"]["SearchParameters"]["TemperatureChangeFrame"] = 0

                # Test policy of the network
                if run != "-1" or self.load_name is not None:
                    strength_exp_json["Experiments"][experiment_name + "_3"] = copy.deepcopy(strength_exp_json["Experiments"][experiment_name])

                    if len(self.experiment_names) == 1:
                        strength_exp_json["Experiments"][experiment_name + "_3"]["OutputDir"] = BIN_PATH + "/" + experiment_name + "/StrengthTest/Network/" + str(run)
                    else:
                        strength_exp_json["Experiments"][experiment_name + "_3"]["OutputDir"] = BIN_PATH + "/CombinedRun/" + experiment_name + "/StrengthTest/Network/" + str(run)
                    
                    strength_exp_json["Experiments"][experiment_name + "_3"]["Run"][1] = 1
    ...
    def split_training_data(self, run):
        with open(os.path.join(DATA_PATH, self.data_file_name + ".csv"), 'r') as data_file:
    ...
    def start(self):
        ...
        for run in range(self.start_run, self.iterations + self.start_run):
            ...
            for experiment_name in self.experiment_names:
                if len(self.experiment_names) == 1:
                    self.experiment_data["Experiments"][experiment_name]["OutputDir"] = BIN_PATH + "/" + experiment_name + "/" + str(run + 1)
                else:
