...
class DataLoader():
    def __init__(self, feat_shape, pred_shape, train_samples, test_samples, shuffle=True, batch_size=32, workers=4):
        ...
        if shuffle:
            self.dataset = self.dataset.shuffle(buffer_size=min(max(train_samples, test_samples), 1000000))
    ...
    def _parse_fn(self, csv_string):
        ...
        d = tf.squeeze(data)
        x, y = d[:-self.pred_shape], d[-self.pred_shape:]
