...
const BuildOrder & NaiveBuildOrderSearch::solve()
{
  ...
  if (checkUnsolvable())
  {
    //!!! PROBLEM UNUSED bool temp = checkUnsolvable();
  ...
  {
    int numCompleted = m_state.getNumTotal(actionType);
  ...
  //!!! PROBLEM UNUSED int maxSupply = m_state.getMaxSupply() + m_state.getSupplyInProgress();
  //!!! PROBLEM UNUSED int currentSupply = m_state.getCurrentSupply();
